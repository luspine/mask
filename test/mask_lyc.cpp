...
//				shpere[i][j][k] = mymask[j][k];
	...
	//
		...
		void *handle  = dlopen("/home/lyc/workspace/mask_lyc/resource/libiimod.so",RTLD_LAZY);
//		void *handle2 = dlopen("./libcfshr.so",RTLD_LAZY);
		void *handle2 = dlopen("/home/lyc/workspace/mask_lyc/resource/libcfshr.so",RTLD_LAZY);
		if(!handle)
		{
			printf("open libiimod error\n");
			cout<<dlerror()<<endl;
			return -1;
		}
		if(!handle2)
		{
			printf("open libcfshr  error\n");
			cout<<dlerror()<<endl;
			return -1;
		}
...
//		FILE (**iiFOpen_t)(const char *filename, const char *mode);
		...
		exitError_t exitError=(exitError_t)dlsym(handle2, "exitError");
		 ...
		 if(!mrc_head_read)
		    {
		        cout<<"mrc_head_read error"<<dlerror()<<endl;
		        dlclose(handle);
		        return -1;
		    }
		 if(!(&iiFOpen))
		    {
		        cout<<"iiFOpen error"<<dlerror()<<endl;
		        dlclose(handle2);
		        return -1;
		    }
		 if(!exitError)
		    {
				cout<<"exitError error"<<dlerror()<<endl;
				dlclose(handle2);
				return -1;
		    }
